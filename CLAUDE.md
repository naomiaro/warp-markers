...
- **`editor.bpm` is display-only once the tick callbacks are set** (since `@dawcore/components` 0.0.23 / `@dawcore/transport` 0.0.12, wfp#407): with `secondsToTicks`/`ticksToSeconds` assigned, the setter never touches the tempo map, and the transport refuses defaulted tick-0 `setTempo` writes on multi-entry maps (returns `false` + console.warn; pass an explicit `atTick` to override). The old "assign `editor.bpm` BEFORE installing tempo events" ordering rule (wfp#406, 97 ms offset) is obsolete — ordering is now style, not safety. On older package versions the trap is real; check the installed version before debugging a constant per-beat offset.
...
- **After any `@dawcore/*` version bump:** `npm install && npm test` (chapter 08's equivalence suite is the upgrade canary — it proves the published TempoMap against chapter-01 math), then check upstream commits to `examples/dawcore-native/beat-map-grid.html` for demo-pattern fixes to port, then verify the BUILT bundle (`vite build` + `vite preview`), never dev only. **Caret ranges pin the patch on 0.0.x** (`^0.0.11` means `<0.0.12`) — new `@dawcore` patches need an explicit range bump in `08-grid-follows-file` and `10-bar-arithmetic` before `npm install` sees them.
- **Upstream's beat-map demo now follows `gridPlanFromBeats`'s lead-meter convention** (wfp#411): its lead-in bars derive from the first DECLARED bar length, not hardcoded 4/4 — the fix came from this repo's chapter 08 (`leadMeter = barLens[0]`), exposed by `bastard.beats`' 2-beat opening bars landing the downbeat at bar 3. Nothing to port back; noted so the upstream-sweep doesn't re-investigate it.
